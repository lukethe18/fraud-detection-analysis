...
CCData['state'] = LabelEncoder().fit_transform(CCData['state'])
CCData['state'] = CCData['state'].apply(lambda x: format(x, 'b'))
CCData['state'] = CCData['state'].apply(lambda x: [int(i) for i in x])
...
# Split Data - always split before encoding or resampling
...
# Initialize the Target Encoder with cross-validation
# 'smoothing' helps prevent overfitting to small group fraud rates.
...
# Finish preparations for SMOTE-NC

# Convert the remaining categorical columns to 'category' dtype
...
# Identify the indices of the remaining categorical features in the new df
categorical_features_indices = [
    X_train_encoded.columns.get_loc(col)
    for col in OTHER_CATEGORICAL_COLS
]

# The HIGH_CARDINALITY_COLS should now be numerical
...
# The X_resampled and y_resampled dataframes are now ready for model training.

# Final Encoding for Model Input
# categorical data usually needs to be numerical i.e 1,2,3..... 600 for scikit

print("\n" + "="*40)
print("FINAL ENCODING BEFORE MODEL TRAINING")
print("="*40)

# One-Hot Encode  remaining categorical columns in the RESAMPLED Training Data
# resampled data MUST be encoded.
X_resampled_final = pd.get_dummies(X_resampled,
                                     columns=OTHER_CATEGORICAL_COLS,
                                     drop_first=False)

# One-Hot Encode the same columns in the UNTOUCHED Test Data
# test data MUST have the same columns as the training data.
X_test_final = pd.get_dummies(X_test_encoded,
                                columns=OTHER_CATEGORICAL_COLS,
                                drop_first=False)

# Ensure Column Alignment **************
# good practice as this step ensures both dataframes have the exact same columns,

# Use reindex to align columns. Fill missing columns (new categories) with 0.
train_cols = X_resampled_final.columns
X_test_aligned = X_test_final.reindex(columns=train_cols, fill_value=0)

# Check for extra columns in train (should be none, but good in practice)
# We use X_resampled_final for training, and X_test_aligned for testing.
...
# Now, use X_resampled_final and X_test_aligned for your Random Forest model:
# rf_model.fit(X_resampled_final, y_resampled)
# rf_model.predict(X_test_aligned)
...
# We set class_weight='balanced' as an extra measure, although the data is already balanced by SMOTE-NC.
# n_estimators could be chosen with  RandomSearchCV or  GridSearchCV instead
...
# Train the model on the SMOTE-NC resampled training data
rf_model.fit(X_resampled_final, y_resampled)
...
# the following are good parameters for imbalanced data (even though we already resampled with SMOTE-NC):
# scale_pos_weight: Helps balance the gradients if the imbalance is still severe.
# use_label_encoder=False and eval_metric='logloss' are standard for binary classification.
# look more into log loss objective function as this is a good thing to know
...
xgb_model = XGBClassifier(
    ...
    random_state=42,
    n_jobs=-1,
    # Since we used SMOTE, we can  set this to 1. If the SMOTE was not  enough, you could try setting this to the ratio of non-fraud/fraud in the original dataset.
    scale_pos_weight=1
)

# Train the model on the SMOTE-NC resampled and fully encoded training data
# No separate scaling is required for tree-based models like XGBoost.
...
# Predict class probabilities (needed for ROC AUC)
# XGBoost outputs probabilities automatically
